...
Phase 2B and 2C include deterministic source discovery with Tavily -> Brave -> direct/ROR fallback, DNS-pinned bounded retrieval, HTML/plain-text normalization, and source/document integration coverage. Phase 2D adds multi-provider structured claim extraction with deterministic segmentation, exact supporting-text promotion, bounded Gemini quality escalation, Groq/OpenRouter Free fallback, provider-specific plus total request budgets, and the fixed provider setup flow. Phase 2E now adds standalone in-memory semantic reconciliation, candidate-traceable final claims, deterministic authority/independence/scope/period gates, category-level unknown versus operational incompleteness, and evidence-bounded explanation fallback under separate reconciliation/explanation budgets. Automated coverage remains offline and deterministic; on 2026-08-16 one explicitly authorized live smoke request succeeded for each configured Tavily, Brave, Gemini, Groq, and OpenRouter connection. Phase 2F full lifecycle orchestration, database migrations, Row Level Security, seed data, persistence, and live Research UI wiring remain intentionally deferred.
