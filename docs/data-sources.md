...
Provider use must be re-checked against current documentation, API terms, licensing, privacy behavior, and rate limits before implementation. Discovery failover is sequential: Tavily -> Brave Search -> direct/structured providers. A 429 is retried only when the provider supplies an explicit `Retry-After` within the one-second discovery retry window; absent, invalid, or longer delays fail over immediately. Within one Research run, persistent discovery failures (`configuration`, authentication, rate-limit, capability, policy) circuit that provider for later category/identity queries instead of re-dispatching the same known-bad provider, while transient timeout/upstream/invalid-response failures are not circuit-broken. Phase 2F distinguishes a completed general-web query from degraded salvage: if Tavily fails but Brave completes (including a clean empty result), discovery may still complete; if both Tavily and Brave fail/unavailable, validated direct/ROR sources are retained as degraded provenance but the category remains operationally incomplete and is not eligible for a final evidence decision or category-level `unknown`. Clean bounded absence is valid only after the required general-web and bounded supplement work completed. Search queries must contain public research context only, not applicant profile or sensitive personal data.
