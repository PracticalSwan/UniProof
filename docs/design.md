...
Phase 6C release configuration keeps the core product anonymous. Hosted Supabase browser/Auth variables are absent because production email delivery is not configured; local Supabase Auth/RLS/persistence evidence remains valid but is not presented as a hosted judge feature. The historical Phase 6C live allowance is exhausted at three accepted calls. The 2026-08-22 reliability release verified executable SHA `f797e0a692f113a29b3f4aa3491a216ead292b2a` with GitHub Actions run `32545347640` and Production deployment `dpl_8pYdBJEyvcohHuMm2e2cXt7cAYm7`. Its separate final-testing allowance consumed one accepted Edinburgh Research-only call: HTTP 200 in about 18.6 seconds, one source, zero claims, sanitized `provider-error` source gap. That demonstrates bounded fail-closed completion under provider pressure, not successful live evidence production. Cumulative accepted-call accounting is therefore 3 historical + 1 final-testing = 4.
