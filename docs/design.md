...
> **As-built architecture reference:** The application source and provider adapters remain, but the hosted project closed on 2026-09-23. Hosting/provider details below describe implementation and historical release decisions, not active resources. See [`operations/project-closure.md`](operations/project-closure.md).
...
| AI | Provider-neutral structured AI adapters: Gemini, Groq `openai/gpt-oss-120b`, OpenRouter `openrouter/free`; former hosted release order was Groq -> OpenRouter with Gemini intentionally unconfigured |
...
For the former public release, `GEMINI_API_KEY` was intentionally left absent because Gemini API terms checked at that time restricted API clients directed toward or likely to be accessed by people under 18, while university applicants can include minors. The former hosted structured-AI chain therefore started at Groq and fell back to OpenRouter. This is a deployment-configuration decision; the Gemini adapter and deterministic contract tests remain in the codebase.

Use fixed server-owned REST endpoints for Gemini Interactions, Groq chat completions, and OpenRouter chat completions through one bounded provider transport pattern: credentials in headers, `redirect: "error"`, explicit timeout/response-byte bounds, best-effort non-blocking failure-body cleanup, and no raw error-body logging. Phase 6B migrated the Gemini adapter from the previously live-smoked `/v1beta/interactions` endpoint to stable `/v1/interactions` while preserving the `steps`/`response_format` contract; the stable endpoint was fixture-tested locally; its historical live validation belongs to the former Phase 6C release. Gemini uses stateless Interactions with `store: false`; Groq uses strict JSON-schema Structured Outputs; OpenRouter requires compatible parameters plus configured data-collection/privacy filtering and records the concrete routed model. All provider output crosses the same portable project schema and Zod validation boundary.
...
Phase 6B added production execution and abuse boundaries without redesigning provider/evidence logic. The application implemented a **120-second Research deadline beneath a 300-second Research-function cap**. Research alone opted into Vercel Node request cancellation through `supportsCancellation:true`; caller cancellation remains `cancelled`, the application deadline becomes `timeout`, completed validated evidence is preserved, and no new retry/fallback/stage dispatch may start after terminal ownership. Raw platform/WAF HTTP 429 and hard-timeout 504 responses are classified before application JSON parsing so hostile/non-JSON bodies cannot leak into UI and no browser retry storm is introduced. During the former Vercel release, one durable WAF rule protected exactly `POST /api/research`, using a source-IP fixed window of 20 requests/60 seconds and returning 429 on excess. The former Vercel release supplied HSTS (`max-age=63072000; includeSubDomains; preload`) at its deployed edge; the application did not add a duplicate application-owned HSTS policy.

The former Phase 6C release configuration kept the core product anonymous. Hosted Supabase browser/Auth variables were absent then because production email delivery had not been configured; local Supabase Auth/RLS/persistence evidence remains valid but was not presented as a hosted judge feature. The historical Phase 6C live allowance was exhausted at three accepted calls, while later reliability testing was authorized separately. The last recorded 2026-08-23 Production executable SHA is `e2ae1414c6f856a0bdeb2aa8a473dcb32215ab3c`, which passed GitHub Actions run `32629551286` and was served by Vercel Production deployment `dpl_GAXGwwiY1KASC3S3Rwt8VisncaNA` at `https://uniproof-beta.vercel.app` until project deletion on 2026-09-23. Bounded live validation at that time returned HTTP 200 for Research, sequential Compare target Research requests, and Guide while preserving sanitized provider gaps and partial-mode behavior under external provider pressure.
