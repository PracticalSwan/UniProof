...
Phase 6B adds production execution and abuse boundaries without redesigning provider/evidence logic: the planned whole-Research application deadline is 210 seconds beneath a 240-second host maximum when the selected Vercel plan supports it; caller cancellation/deadline stops new retries/fallback dispatch while preserving already validated partial evidence. Deployment-generated 429/timeout failures become sanitized public transport outcomes. A durable Vercel WAF rule scoped only to `POST /api/research` is the preferred distributed abuse control, initially observed in Log mode and enforced only after current pricing/plan behavior and normal judge traffic are verified. Production hardening also covers environment separation, least-privilege CI, provider-policy revalidation, TLS/CSP/cache/session behavior, and conservative HSTS without `includeSubDomains` or preload by default.
