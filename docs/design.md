...
Phase 6C release configuration keeps the core product anonymous. Hosted Supabase browser/Auth variables are absent because production email delivery is not configured; local Supabase Auth/RLS/persistence evidence remains valid but is not presented as a hosted judge feature. Preview deployment verified the hardened executable UI with 104/104 deterministic Research/Compare/Guide browser acceptance cases. Release SHA `21d645baaf9eca381a167246d22538c23bb29427` then passed GitHub Actions run `32367630411` and was deployed as Vercel Production deployment `dpl_3BppbKoR2sEshhGqoKStotZ7xyhN`, serving the canonical `https://uniproof-beta.vercel.app`. Deterministic Production checks verified the core routes, nonce CSP/cache/security headers, edge HSTS, WAF state, same-origin client bundles, absence of configured secret values/provider internals in those bundles, and no observed error/5xx runtime logs in the post-deploy smoke window. Release screenshots were captured from the verified hosted UI using intercepted deterministic Research fixtures. The three-call live Research budget is exhausted; the third call returned a schema-valid but operationally incomplete admissions dossier and exposed two resilience defects that were fixed deterministically: source-gap claims are preserved but downstream-non-definitive, and program-scoped discovery always retains the catalog-owned official program page even when web discovery succeeds. Devpost publication remains the only deliberate external hold until the final demo video is supplied and approved.
