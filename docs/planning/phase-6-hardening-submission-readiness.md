...
> **Historical implementation and release record:** Phase 6 release activity ended before the project closure on 2026-09-23. The Vercel project and its WAF configuration have been removed; these plans and deployment steps are retained as evidence and are not current instructions. See [`../operations/project-closure.md`](../operations/project-closure.md).
...
- The former public release had durable Vercel WAF enforcement, verified Preview and Production security/header behavior, bounded hosted provider configuration, exact-SHA GitHub CI/Production traceability, and release screenshots. Hosted Auth/save is intentionally disabled rather than partially configured. The only remaining release gate is the separately held Devpost video/submission step.
...
**Observed Phase 6C release-gate evidence (2026-08-20):** the anonymous release satisfied every pre-Devpost item above. Hardened Preview acceptance passed 104/104 deterministic Research/Compare/Guide browser cases; executable SHA `21d645baaf9eca381a167246d22538c23bb29427` passed GitHub Actions run `32367630411`; Vercel Production deployment `dpl_3BppbKoR2sEshhGqoKStotZ7xyhN` was metadata-bound to that SHA and served the canonical origin until the project was deleted on 2026-09-23; the exact Research WAF rule was enabled then and was removed with the project on 2026-09-23; and deterministic Production route/header/security/client-bundle/log checks passed without a fourth live Research call. Hosted Auth/save was intentionally absent then. At the 2026-08-23 check, the release state was `submission-ready / pending final video`, not Devpost-submitted. Current Devpost state is not verified.
...
**Observed 2026-08-23 provider-resilience evidence:** executable baseline `e2ae1414c6f856a0bdeb2aa8a473dcb32215ab3c` passed GitHub Actions run `32629551286` and was served by Production deployment `dpl_GAXGwwiY1KASC3S3Rwt8VisncaNA` at the canonical origin until the project was deleted on 2026-09-23. The application deadline is 120 seconds beneath the unchanged 300-second host cap. Separately authorized browser checks, spaced with deliberate cooldowns, returned HTTP 200 for Research, both serialized Compare target Research requests, and Guide; provider gaps remained sanitized and modes degraded to partial/non-definitive evidence rather than generic whole-mode failure. At the 2026-08-23 check, Devpost was recorded as unsubmitted pending the final video and explicit submission authorization; its current state is not verified.
