...
Implementation status (updated 2026-08-20): Phase 5 Guide Mode remains implemented and browser-verified, and Phase 6 has since added optional local Auth/private snapshot persistence plus the deployed release controls without changing Guide's applicant-to-provider privacy boundary. The public judge deployment intentionally leaves hosted Auth/save disabled; durable Research WAF, exact-SHA CI/Production, deployed HSTS/security headers, and bounded live-provider smoke are now verified in Phase 6C evidence.
...
- Require a durable/deployment-layer distributed abuse control for the expensive public Research endpoint before public release; local process memory, browser single-flight behavior, provider attempt budgets, and hidden local test modes do not satisfy this requirement. The deployed Phase 6C Vercel control is one enabled rule for exact `POST /api/research`, source-IP fixed window, 20 requests/60 seconds, returning HTTP 429 on excess.
- Bound each accepted Research run with a server-owned 120-second application deadline beneath a 300-second Research-function cap; caller cancellation remains distinct from deadline timeout, no new retry/fallback/stage dispatch may begin after terminal ownership, and completed validated evidence must be preserved as truthful partial output where possible. Actual Vercel request-cancellation delivery remains a deployed verification requirement.
...
Implementation status (updated 2026-08-23): Phase 6A identity/ownership/persistence and Phase 6B production hardening are implemented and verified. Phase 6C deployed the anonymous judge release with Tavily/Brave discovery, Groq/OpenRouter structured AI, one enforced Vercel Research WAF rule, Node 22.x / 300-second Research host configuration, a 120-second application-owned Research deadline, canonical TLS/HSTS/CSP/cache/security headers, exact-SHA GitHub CI, and Vercel Production traceability. Final testing additionally makes arbitrary general-web support non-corroborating, binds Magic Links to the initiating browser, recomputes persisted Guide/Comparison derivations, authenticates before private body buffering, clears account-derived restores, and hardens UI error/focus behavior. The historical Phase 6C live Research allowance remains exhausted at 3/3; later 2026-08-23 reliability testing was separately authorized and returned bounded HTTP 200 Research/Compare/Guide responses while preserving sanitized provider gaps and partial-mode behavior. Hosted Supabase Auth/save remains intentionally absent from the judge release, and Devpost final submission remains blocked on the final video and explicit submission authorization.
