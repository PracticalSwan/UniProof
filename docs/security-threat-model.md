...
- **Magic Link login CSRF/account swapping:** a one-time token alone must not choose the account in another browser. A short-lived same-browser intent cookie/state pair is consumed before `verifyOtp`; cross-context, missing, mismatched, and reused intent fails closed.
...
- **Persisted owner tampering/content-integrity confusion:** RLS establishes ownership, not truthfulness of an owner's JSON. Every DB row is untrusted and must pass strict version/runtime/current-catalog validation; version-1 Guide assessment/planning and Comparison scoring/trade-offs are recomputed and must equal the stored derivation. Do not add HMAC/service-role signing merely to stop a user modifying their own private snapshot.
- **Restore confused-deputy/stale overwrite:** saved payloads cross routes only through one account-bound memory-only single-consume handoff. Account change/signout clears both pending and already restored account-derived state in Research, Compare, and Guide; exact kind/account/operation ownership is rechecked after awaits, so delayed restore cannot publish after sign-out.
...
- **Storage/resource abuse:** private identity is verified before POST body buffering; save requests then enforce actual streamed-byte bounds, strict runtime schema/version validation, one bounded maximum-20 metadata list, and a database-race-safe per-owner artifact cap without silent eviction.
...
Phase 4/5 evidence remains part of the reviewed baseline. Phase 6A identity/ownership/persistence evidence was observed locally on 2026-08-19, then final testing on 2026-08-21 repeated the stack from zero and passed pgTAP 40/40 plus Auth/Saved 12/12 with same-browser Magic Link binding, delayed-restore rejection, concurrent cap enforcement, ambiguous-save retry blocking, and private profile/Research sign-out clearing. The public release intentionally leaves hosted Supabase Auth/save disabled, so no hosted private-state claim is made. The historical Phase 6C live budget remains exhausted at 3/3; later 2026-08-23 reliability checks were separately authorized and returned bounded HTTP 200 Research/Compare/Guide responses while preserving sanitized provider gaps and non-definitive partial evidence.
...
- same-browser Magic Link intent success plus cross-browser rejection before token consumption;
...
- pre-auth private-body rejection, ambiguous Save/Delete `5xx` handling, derived-result tamper rejection, delayed-restore account-change rejection, Research restore clearing, and concurrent owner-cap insertion;
