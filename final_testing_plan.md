...
> **Historical plan; do not execute as current instructions.** This plan contains now-stale steps to connect GitHub and Vercel, deploy, run production checks, and prepare Devpost. The Vercel project was removed on 2026-09-23, the connected Supabase account has no UniProof project, and the current Devpost entry/submission state is unverified. Keep the source and historical evidence; any future external action requires new explicit owner authorization. See [`docs/operations/project-closure.md`](docs/operations/project-closure.md).
